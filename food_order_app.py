...
def read_user_csv():
    with open("user_data.csv", "r") as fp:
        ...
        for line in csv_reader:
            user_l.append(User(line[0], line[1], line[2], line[3], line[4], line[5]))

def write_user_csv():
    with open("user_data.csv", "w", newline="") as fp:
        csv_writer = csv.writer(fp)
        for i in user_l:
            csv_writer.writerow([i.name, i.phone, i.address, i.email, i.password, i.is_admin])

def read_item_csv():
    with open("item_data.csv", "r") as fp:
        csv_reader = csv.reader(fp)
        for line in csv_reader:
            item_l.append(Item(line[0], line[1], line[2], line[3], line[4], line[5]))
...
def show_food_items():
    ...
    for item in item_l:
        print("           ")
        print(item.food_id, "\t", item.name, "\t\t", item.quantity, "\t\t", "₹"+item.price)

def calculate_total():
    total_amt = 0
    for order in order_l:
        total_amt += int(order.price) - (int(order.price) * int(order.discount) / 100)
    return total_amt

def new_order():
    show_food_items()
    print("*"*70)
    order_input = input("Enter Food ID you wish to order separating using comma: ")
...
def add_history():
    ...
    for order in order_l:
        order_history_l.append(OrderHistory(history_id, logged_in_user.email, order.food_id))
    write_order_history()
...
def write_item_csv():
    with open("item_data.csv", "w", newline="") as fp:
        csv_writer = csv.writer(fp)
        for i in item_l:
            csv_writer.writerow([i.food_id, i.name, i.quantity, i.price, i.discount, i.stock])
...
def edit_item():
    ...
    for item in item_l:
        if food_id == item.food_id:
            ...
            print("(3) PRICE")
            print("(4) DISCOUNT")
            update_item = input("Choose Option to Update or press 5 to go back: ")
            if update_item == "1":
